...
function updateCarousel() {
  items.forEach((item,index)=>{
    if(index===currentIndex){item.classList.add('center');} else {item.classList.remove('center');}
  });
...
prev3.addEventListener('click',()=>{currentIndex3=(currentIndex3-1+items3.length)%items3.length;updateCarousel3();});
next3.addEventListener('click',()=>{currentIndex3=(currentIndex3+1)%items3.length;updateCarousel3();});
updateCarousel3();